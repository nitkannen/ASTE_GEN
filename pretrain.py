...
def get_optimizer_scheduler(model, train_dataloader, epochs):
    ...
    optimizer = AdamW(optimizer_grouped_parameters, lr=2e-7, eps = 1e-8) # 2e-7
...
if __name__ == '__main__':
  ...
  loader = DataLoader(
                      Contrastive_Dataset, collate_fn = collate_fn ,shuffle = True,  batch_size=
                      args.batch_size
            )
  ...
  for epoch in range(epochs):
    ...
    if(epoch == 10 or epoch == 8 or epoch == 4 or epoch == 6 or epoch == 12 or epoch == 3 or epoch == 15): ## needs editing
